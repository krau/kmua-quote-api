...
def generate_quote_img(avatar: bytes, text: str, name: str) -> bytes:
    logger.debug(f"Generating quote image for [{name}], [{text}]")
    text = text.replace("\n", " ")
    font_path = str(Path(__file__).resolve().parent / "public" / "TsukuA.ttc")
    font_size = 42
    font = ImageFont.truetype(font_path, font_size)
    base_img = Image.open(Path(__file__).resolve().parent / "public" / "base.png")
